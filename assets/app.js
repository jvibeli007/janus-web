...
function renderMetrics() {
  ...
  const rows = [
    ...
    ["Partners", metrics.partners || 0, "共同出現實體"],
...
function renderFiveP() {
  ...
  const topPartners = celebrity.partners.slice(0, 4).map(row => row.partner).join("、") || "尚無 partner";
  const boxes = [
...
function renderSourceFilter() {
  ...
  currentCelebrity().partners.forEach(row => row.sourceTypes.forEach(type => sources.add(type)));
...
function filteredPartners() {
  ...
  return currentCelebrity().partners.filter(row => {
...
function answer(input) {
  ...
  if (lower.includes("首次") || lower.includes("first")) {
    const firstRows = celebrity.partners.filter(row => row.first);
    return firstRows.length
      ? `本次 snapshot 首次出現：${firstRows.map(row => `${row.partner}（${row.mentions} 次）`).join("、")}。`
      : "目前沒有新的首次出現 partner。歷史檔越多，這個判斷會越準。";
  }
  ...
  if (lower.includes("提及") || lower.includes("top")) {
    const top = [...celebrity.partners].sort((a, b) => b.mentions - a.mentions).slice(0, 6);
    return top.length ? `提及次數 Top：${top.map(row => `${row.partner} ${row.mentions} 次`).join("；")}` : "目前沒有 partner 提及資料。";
  }
  const partner = celebrity.partners.find(row => lower.includes(row.partner.toLowerCase()));
  if (partner) {
